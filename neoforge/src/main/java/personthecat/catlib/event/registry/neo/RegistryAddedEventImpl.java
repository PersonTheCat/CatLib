package personthecat.catlib.event.registry.neo;
...
public class RegistryAddedEventImpl {
    ...
    private static <T> LibEvent<RegistryAddedCallback<T>> create(final ResourceKey<Registry<T>> key) {
        ...
        ((MojangRegistryHandle<T>) handle).getRegistry()
            .addCallback((AddCallback<T>) (registry, id, k, v) ->
                event.invoker().onRegistryAdded(handle, k.location(), v));
        return event;
    }
